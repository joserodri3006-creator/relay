...
run("PostgreSQL tenant isolation", () => {
  ...
  it("erzwingt RLS auf allen tenantgebundenen Fachtabellen", async () => {
    const expected = ["actor_capabilities","actor_identities","actors","audit_entries","channel_authorizations","commitments","conversations","domain_events","handoffs","ingress_receipts","integration_issues","interactions","oauth_authorization_flows","outbox_events","pilot_channel_accounts","pilot_onboarding_requests","timeline_events"];
